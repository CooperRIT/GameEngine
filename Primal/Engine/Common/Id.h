...
namespace primal::id
{
	...
	namespace internal
	{
		struct id_base
		{
			constexpr explicit id_base(primal::id::id_type id) : _id(id) {}
			constexpr operator primal::id::id_type() const { return _id; }
...
#define DEFINE_TYPE_ID(name)
	...
	{
		constexpr explicit name(id_type id) : id_base{ id}{}
			
		constexpr name() : id_base{id_mask} {}
	};
#else
#define DEFINE_TYPE_ID(name) using name = internal::id_type
#endif
